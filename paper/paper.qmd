...
[Model] The standard error of each $\beta_b$ was calculated from the variation between calendar months rather than between days, because a wet week raises a beach's levels for several days running; this roughly doubles the width of the intervals compared with treating every day as independent. Each estimate is reported with a 95% interval: if the analysis were repeated over many different runs of summers, about 95% of such intervals would contain the true gap. Two beaches were judged to differ when a gap as large as the one observed would arise less than 5% of the time if they were in fact alike. Testing all 45 pairs this way would produce about two false differences by chance, so the comparisons were corrected by Holm's method, which raises the bar each comparison must clear so that the chance of any false difference stays near 5%.

[Consecutive days] To measure how well a posting based on the previous day's samples describes the water on the day it is read, each beach-day was paired with the same beach's result on the day before. Days either side of a gap in sampling were not paired, which leaves 18,998 pairs. Treating yesterday's result as today's advice gives four outcomes: a warning when today's water is over the limit or under it, and an all-clear when today's water is over or under it. The counts of these outcomes form a two-by-two table, often called a confusion matrix. Two ratios summarise the table from a swimmer's point of view: the share of days over the limit that a warning caught, and the share of warnings that were needed. Overall accuracy, the share of days on which the advice was right, is not used, because most days are under the limit and a rule that never posted a warning would score 86%.
