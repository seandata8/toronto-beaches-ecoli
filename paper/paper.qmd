...
[Consecutive days] Because results take about a day, the advice on display at a beach on any day, called here the posting day, is based on the samples taken the day before. To measure how often that advice was right, each posting day was paired with the previous day at the same beach, using only days that were consecutive (18,998 pairs). A warning was taken to be posted if the previous day's geometric mean was over the limit, and no warning if it was not. That advice was then compared with the water sampled on the posting day itself to assess if the advice matched the water quality on the posting day.
...
*[Yesterday vs today] Placeholder: @tbl-warning; name the four outcomes the table shows (correct warning, false warning, missed warning, correct no-warning), each tied to the posting day's water; avoid the term "confusion matrix" or explain it; of the posting days with water over the limit, 37% had a warning posted; of the warnings posted, 37% were correct; 14% of posting days have water over the limit overall, 37% after a warning and 10% with no warning (about 3.5 times as likely). Leave out overall accuracy.*

```{python}
#| label: tbl-warning
#| tbl-cap: "Advice posted at each beach, based on the previous day's samples, against the water on the posting day, 2007 to 2026 (18,998 pairs of consecutive days). A warning is posted when the previous day's geometric mean was above Toronto's limit of 100 *E. coli* per 100 mL. Each cell gives the number of pairs and its share of all pairs."
#| tbl-colwidths: [34, 33, 33]
#| output: asis

confusion = pl.read_csv("../other/results/warning-confusion-matrix.csv")
ADVICE_LABELS = {"Warning": "Warning posted", "All-clear": "No warning posted"}

print("| Advice | Water over the limit on the posting day | Water under the limit |")
print("|:--|--:|--:|")
for row in confusion.iter_rows(named=True):
    print(
        f"| {ADVICE_LABELS[row['advice']]} "
