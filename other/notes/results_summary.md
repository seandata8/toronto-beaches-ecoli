...
**31 of the 45 pairs differ** after Holm's correction. The 14 that cannot be separated are Marie Curtis and Sunnyside, Centre Island and Kew Balmy, and 12 pairs among the six cleanest beaches, where the gaps are 0.01 to 0.07 on the log10 scale.

**Grouping days for the standard errors (decided 26 September 2026).** Days are grouped by month, with the partial months at each end of the season (late May, early September) merged into June and August: 60 groups rather than 96. This was one of two groupings compared on 25 September; with plain calendar months Bluffer's against Cherry also differed (32 pairs). The paper uses the merged grouping only, so the result is 31 pairs.
...
**Consecutive days are related, so the standard errors group days by month.** A wet week raises a beach for several days running: one day's level correlates with the next at 0.42 (Question 2). Fitting the same model with every day treated as independent gives standard errors 1.4 to 2.3 times smaller (median 2.0, with partial months merged; `other/results/beach-model-standard-errors.csv`). So the grouping roughly doubles the width of the intervals, and the data carry about as much information as a quarter as many independent days. Ignoring it would make the p-values far too small.
