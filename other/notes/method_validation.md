...
- Grouping days by month widens the standard errors by a median of 1.36 times, as the built-in carry-over implies. (This replaced a Poisson check on 25 September 2026; the old check pooled beaches built at different levels, so it measured their differences as much as any clustering. The real data give 1.90, so real days are more closely related than the simulation assumes.)
